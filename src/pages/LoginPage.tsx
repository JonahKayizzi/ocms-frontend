...
import { storeToken, getToken, getUserInfo, isAdmin } from "../utils/jwtUtils";

export default function LoginPage() {
  const navigate = useNavigate();
  const location = useLocation() as {
    state?: { warning?: string; success?: string };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (token) {
        ...
        const userIsAdmin = userInfo?.isAdmin || isAdmin(token);
        const redirectPath = userIsAdmin ? "/admin" : "/dashboard";
        navigate(redirectPath, { state: { success: "You are logged in." } });
        return;
      }
      ...
      // For demo mode, redirect regular users to dashboard
      navigate("/dashboard", {
        state: { success: "You are logged in (demo mode)." },
      });
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div className="mb-6 text-center">
          <div className="mx-auto w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center text-white text-xl">
            📘
          </div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Sign in with your SMS or AIM account
          </p>
          ...
          {location.state?.success && (
            <p className="mt-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-3 py-2">
              {location.state.success}
            </p>
          )}
        </div>
