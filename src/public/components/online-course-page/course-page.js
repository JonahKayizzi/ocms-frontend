...
export default function CoursePage() {
  ...
  const username =
    (search.get("participantId") || usernameToken || "").toString() || null;

  const { data: course } = useGetCourseByIdQuery(courseId, { skip: !courseId });
  const { data: modules = [] } = useGetModulesByCourseQuery(courseId, {
    skip: !courseId,
  });
  const { data: lessons = [] } = useGetLessonsByCourseQuery(courseId, {
    skip: !courseId,
  });
  const { data: materials = [] } = useGetMaterialsByCourseQuery(courseId, {
    skip: !courseId,
  });
  const { data: assessments = [] } = useGetAssessmentsByCourseQuery(courseId, {
    skip: !courseId,
  });
  const { data: enrollmentCountResp } = useGetEnrollmentCountQuery(courseId, {
    skip: !courseId,
  });
  const { data: enrolledResp, refetch: refetchEnrollment } = useIsEnrolledQuery(
    { courseId, participantId: username || "" },
    { skip: !courseId || !username }
  );
  const { data: progressResp, refetch: refetchProgress } =
    useGetCourseProgressQuery(
      { courseId, participantId: username || "" },
      { skip: !courseId || !username }
    );
  ...
  const { data: userAttempts = [] } = useGetUserQuizAttemptsQuery(
    username || "",
    { skip: !username }
  );
  ...
  const currentLesson = useMemo(
    () =>
      lessons && lessons.length > 0
        ? lessons[Math.min(Math.max(currentIndex, 0), lessons.length - 1)]
        : null,
  ...
  const goNext = () =>
    setCurrentIndex((idx) => (idx < totalLessons - 1 ? idx + 1 : idx));
